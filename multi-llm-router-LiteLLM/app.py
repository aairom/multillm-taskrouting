#!/usr/bin/env python3
"""
app.py  —  Multi-LLM Task Router with LiteLLM AI Gateway  (demo application)

Runs a FastAPI server with three endpoints:
  POST /route      →  route and execute a prompt via LiteLLM Router → Ollama
...
Demonstrates the scenario:
  ...
  Sub-task A (Documentation)  → model::light    (IBM Granite  — cheap, fast)
  Sub-task B (Code Generation) → model::heavy   (Mistral AI   — powerful)

The key difference from the plain Ollama version:
  All LLM calls go through the LiteLLM Router, which adds:
    • Automatic retries + exponential back-off
    • Per-deployment cooldown after repeated failures
    • Transparent fallback chain
    • Uniform OpenAI-compatible response parsing
"""
...
# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="Multi-LLM Task Router — LiteLLM Edition",
    description=(
        "Routes sub-tasks to the cheapest model tier that meets a quality threshold. "
        "All LLM calls are dispatched through the LiteLLM AI Gateway Router to Ollama."
    ),
    version="1.0.0",
)
...
async def route_prompt(req: RouteRequest):
    """
    Route a prompt (optionally compound) to the appropriate LLM tier(s) via LiteLLM.
...
def run_demo_scenario() -> None:
    ...
    Demonstrates how the same prompt is split across 4 LLM providers via
    the LiteLLM AI Gateway Router:
    ...
    W = 80
    print("=" * W)
    print("  Multi-LLM Task Router  [LiteLLM Edition]  —  Demonstration Scenario")
    print("=" * W)
    print(f"\nPrompt:\n  {DEMO_PROMPT}\n")

    # ── Gateway info ──────────────────────────────────────────────────────────
    ollama_base = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
    print(f"LiteLLM Gateway → Ollama at: {ollama_base}")
    print("\nModel tier mapping (LiteLLM alias → Ollama model):")
    for m in MODEL_REGISTRY:
        real = TIER_TO_MODEL.get(m.label, "?")
        print(f"  {m.label:<20s}  ({m.provider:<14s})  →  ollama/{real}")
    print()
    ...
    # ── Step 4: Display ───────────────────────────────────────────────────────
    print("\n" + "=" * W)
    print("  ROUTING TABLE  (via LiteLLM AI Gateway)")
    print("=" * W)
    print(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Multi-LLM Task Router — LiteLLM Edition")
    parser.add_argument(
        "--mode",
        choices=["server", "demo"],
        default="demo",
